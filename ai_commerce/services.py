...
Architecture (rule-based + NLTK, per Step 12 revision â€” no external/paid AI
...
change â€” everything else (views, models, cart integration, the public
...
def _effective_max_recommendations(session):
    ...
    Scales basket size with family_size when provided â€” a party of 10
...
def _ensure_nltk_data():
    ...
    that hasn't run the download command yet â€” auto-downloads quietly if
...
def _detect_light_food_signal(text):
    ...
    heavy categories in favor of curated occasion categories â€” e.g. so
...
def parse_natural_language_query(raw_query):
    ...
    matching â€” no external AI service involved.
...
_GOAL_QUALITY_MAP = {
    ...
    # healthy_living/party_shopping/weekly_groceries are handled via
    # _GOAL_KEYWORD_MAP instead — no quality-tier signal for those.
}
...
_DISH_KEYWORD_CATEGORY_MAP = {
    ...
    # Step B additions â€” occasion-to-category bridges, same simple
...
def _detect_dish_categories(raw_query):
    ...
    keyword) and the actual catalog structure â€” e.g. "jollof rice" implies
    ...
    matching on the full phrase, not per-token â€” same rule-based
...
def _effective_quality_preference(session):
    ...
    price_sensitivity so the same filter logic applies uniformly. For

    Mode 3 (shop by goal), derive an equivalent from the selected goal —

    save_money/premium_quality goals carry no keyword signal in

    _GOAL_KEYWORD_MAP by design, so without this branch they were

    previously no-ops in scoring (confirmed bug, fixed here).
    ...
    if session.mode == 'shop_by_goal' and session.goal:

        return _GOAL_QUALITY_MAP.get(session.goal)
...
def _balanced_candidate_pool(combined_queryset, categories, max_candidates=MAX_CANDIDATES):
    ...
    on the final ranked list â€” applied here one step earlier, at
...
# shopping_purpose is free text on ANY mode (not just guided_planner) â€”
    ...
    # Category and keyword matches combine as OR, not AND â€” a request like
    ...
    # only checks product_name/description â€” NOT category_name â€” since
...
def _score_and_explain_one(session, product, categories, keywords, quality_preference,
                            ...
                            purchase_categories=None, product_order_counts=None):
    ...
    â€” this is the fix for occasion-detected products losing arbitrary
    ...
    if occasion_matched:
        ...
        if light_food_signal:
            ...
            score -= 2  # de-prioritized, not excluded â€” still a valid catalogue option
    ...
    product_order_counts = product_order_counts or {}

    purchase_categories = purchase_categories or set()

    if product.id in product_order_counts:

        times = product_order_counts[product.id]

        score += 1.5

        relevance_found = True

        reasons.append(f"you've ordered this before ({times} time{'s' if times != 1 else ''})")

    elif category_name and category_name in purchase_categories:

        score += 1

        relevance_found = True

        reasons.append(f"you've purchased from {category_name} before")
...
def _rank_and_explain(session, candidates):
    ...
    purchase_categories, product_order_counts = _get_customer_purchase_signal(session.customer)
...
def generate_shopping_recommendations(session):
    ...
    The single entry point for producing a ranked, explained basket â€”
    ...
    Flow:
      ...
      1. get_candidate_products(session) â€” rule-based, zero AI

      2. _rank_and_explain(session, candidates) â€” weighted scoring +
...
# multiplier is applied. Named constants, not scattered magic numbers â€”
...
def calculate_credit_recommendation(customer):
    ...
    replaced by an ML model later without changing any caller â€” same
    ...
    Persists and returns a new CreditAssessment row (append-only â€” full
...
def get_reorder_suggestions(customer, limit=5):
    ...
    Pure read/aggregation against existing order history â€” no new model,
...
def _get_customer_purchase_signal(customer, limit=5):

    """

    Lightweight purchase-history signal for RANKING existing candidates —

    not a retrieval filter, and not the full order history. Reuses

    get_reorder_suggestions()'s existing aggregation (no new query

    pattern). Returns (frequent_categories, product_order_counts):

      - frequent_categories: set of Category.category_name strings the

        customer has bought from before

      - product_order_counts: {product_id: times_ordered} for their

        most frequently bought products



    Returns (set(), {}) for guests / no customer, so every caller stays

    safe without an extra None-check.
    ...
    product_order_counts = {s["product"].id: s["times_ordered"] for s in suggestions}

    return frequent_categories, product_order_counts
...
def build_validated_basket(recommendations, budget=None):
    ...
    (real products, real prices, real stock â€” nothing computed here is
    ...
    AI_PROVIDER â€” only the phrasing differs, never the facts.
